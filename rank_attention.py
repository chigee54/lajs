...
from models import RankAttention
...
def train(model, train_dataloader, valid_dataloader, args):
    ...
    for epoch in tqdm(range(args.rank_epoch_num)):
        ...
        if best < ndcg30:
            best = ndcg30
            torch.save(model, args.rank_output_path + '/' + f"RankAttention.pt")
...
def ranking(args=None,
    ...
    if mode == 'test':
        ...
        model = torch.load(args.rank_model_path)
        result = test_result(test_dataloader, model, args.device)
        return result
    elif mode == 'train':
        logging.basicConfig(filemode='a',
                            level=logging.INFO,
                            filename=args.rank_output_path + '/{}.log'.format("RankAttention"),
        ...
        model = RankAttention(args.input_size, args.hidden_size)
        train(model, train_dataloader, valid_dataloader, args)
    else:
        raise ValueError('Ranking Mode Error!')
